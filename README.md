change project name in package.json name and also on the travis.yml file beforescript line -test
create database with name, createdb projectname
and create database with name, createdb projectname-test
the process.env.DATABASE_URL || is saying if there is a database name in the current environment place, use that. Otherwise assume the database is on our local machine and use localhost.
other ternary says: When you connect to the database, please connect to the tesing database
...
```
git remote add cookpot https://github.com/dldimartino/cookpot.git
git fetch cookpot
git merge cookpot/main (was master)
```
